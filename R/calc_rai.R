...
stats <- c('sum')
admin_pop <- exactextractr::exact_extract(pop,admin, stats) %>% as.data.frame()
names(admin_pop) <- c("pop1")
admin <- cbind(admin,admin_pop)

comb_admin_urban_pop <- exactextractr::exact_extract(pop, comb_admin_urban, stats) %>% as.data.frame()
...
comb_admin_line_pop <- exactextractr::exact_extract(pop, comb_admin_line, stats) %>% as.data.frame()
...
comb_line_urban_pop <- exactextractr::exact_extract(pop, comb_line_urban, stats) %>% as.data.frame()
